...
#$(shell if riscv64-unknown-elf-objdump -i 2>&1 | grep 'elf64-big' >/dev/null 2>&1; \
...
#todo old ASFLAGS = -ggdb -march=rv64ima -mabi=lp64
ASFLAGS = -g -march=rv64ima_zicsr -mabi=lp64

CFLAGS  = -Og -ggdb #-Wall -Werror
CFLAGS += -nostdlib
...
LDSCRIPT = kernel.ld
LDFLAGS  = -z max-page-size=4096 --script ${LDSCRIPT} --no-relax
